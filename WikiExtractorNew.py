...
from new import extract_infobox, extract_image_and_caption, split_by_first_level_pipes
...
options = SimpleNamespace(
    ...
    #
    acceptedNamespaces = ['w', 'wiktionary', 'wikt', 'File', 'InfoboxHeader'],
...
class Extractor(object):
    ...
    def expand(self, wikitext):
        ...
        # GET INFOBOX IF PRESENT
        infobox_image_caption = ""
        infobox = extract_infobox(wikitext)
        
        if infobox is not None:
            infobox_image, infobox_caption = extract_image_and_caption(infobox)
            if infobox_image is not None:
                extension = infobox_image.split('.')[-1].lower()
                if extension in options.imageExtensions and infobox_caption is not None:
                    infobox_image_caption = '[[InfoboxHeader:{infobox_image}|thumb|{infobox_caption}]]'.format(
                        infobox_image=infobox_image, infobox_caption=infobox_caption)
        ...
        # leftover
        res += wikitext[cur:]
        if infobox_image_caption:
            res = infobox_image_caption + res.strip()
...
def replaceInternalLinks(text, first_level=True):
    """
    ...
    for s, e in findBalanced(text):
        ...
        if pipe < 0:
            ...
            # check if file or infobox
            if first_level and (title.startswith('File:') or title.startswith('InfoboxHeader:')):
                components = split_by_first_level_pipes(inner)
                label = components[-1]
                label = replaceInternalLinks(label, first_level=False)
                label = replaceExternalLinks(label)
                label = magicWordsRE.sub('', label)
                if "thumb" in [x.strip() for x in components]:
                    label = 'thumb|' + label
            else:
                # find last |
                curp = pipe + 1
        ...
        res += text[cur:s] + makeInternalLink(title, label, first_level) + trail
        cur = end
    return res + text[cur:]
...
def makeInternalLink(title, label, first_level=True):
    colon = title.find(':')
    ...
    if colon == 0:
        ...
        if colon2 > 1 and title[colon + 1:colon2] not in options.acceptedNamespaces:
            return ''
    if first_level and options.keepLinks:
        if title.startswith('File:') or title.startswith('InfoboxHeader:'):
            extension = title[title.rfind('.') + 1:]
            if not label.startswith("thumb|"):
                return ''
            if not extension or extension.lower() not in options.imageExtensions:
                return ''
